...
{
	[AddComponentMenu("Network/NetworkLobbyManager2")]
	public class NetworkLobbyManager2 : NetworkManager
	{
		...
		[SerializeField] NetworkLobbyPlayer2 m_LobbyPlayerPrefab;
		...
		public NetworkLobbyPlayer2[] lobbySlots;
		...
		public NetworkLobbyPlayer2 lobbyPlayerPrefab { get { return m_LobbyPlayerPrefab; } set { m_LobbyPlayerPrefab = value; } }
		...
		{
			var lobbyPlayer = lobbyPlayerGameObject.GetComponent<NetworkLobbyPlayer2>();
			if (lobbyPlayer == null)
			{
		...
		static int CheckConnectionIsReadyToBegin(NetworkConnection conn)
		{
			...
			foreach (var player in conn.playerControllers)
			{
				if (player.IsValid)
				{
					var lobbyPlayer = player.gameObject.GetComponent<NetworkLobbyPlayer2>();
...
//			OnLobbyServerConnect(conn);
		...
		public override void OnServerAddPlayer(NetworkConnection conn, short playerControllerId)
		{
			...
			var newLobbyPlayer = newLobbyGameObject.GetComponent<NetworkLobbyPlayer2>();
		...
		public override void OnServerRemovePlayer(NetworkConnection conn, PlayerController player)
		{
			...
			byte slot = player.gameObject.GetComponent<NetworkLobbyPlayer2>().slot;
			...
			foreach (var p in lobbySlots)
			{
				if (p != null)
				{
					p.GetComponent<NetworkLobbyPlayer2>().readyToBegin = false;
		...
		public override void ServerChangeScene(string sceneName)
		{
			if (sceneName == m_LobbyScene)
			{
				foreach (var lobbyPlayer in lobbySlots)
				{
					...
					if (uv.connectionToClient.GetPlayerControllerExtension(uv.playerControllerId, out playerController))
					{
					...
					if (NetworkServer.active)
					{
						...
						lobbyPlayer.GetComponent<NetworkLobbyPlayer2>().readyToBegin = false;
		...
		void OnServerReadyToBeginMessage(NetworkMessage netMsg)
		{
			...
			var lobbyPlayer = lobbyController.gameObject.GetComponent<NetworkLobbyPlayer2>();
		...
		void OnServerSceneLoadedMessage(NetworkMessage netMsg)
		{
			...
			PlayerController lobbyController;
			if (!netMsg.conn.GetPlayerControllerExtension((short)s_SceneLoadedMessage.value, out lobbyController))
			{
		...
		public override void OnStartServer()
		{
			...
			{
				lobbySlots = new NetworkLobbyPlayer2[maxPlayers];
			}
...
public static class NetworkConnectionExtensionMethods {
	...
	public static bool GetPlayerControllerExtension(this NetworkConnection value, short playerControllerId, out PlayerController playerController){
		MethodInfo dynMethod = value.GetType().GetMethod("GetPlayerController", BindingFlags.NonPublic | BindingFlags.Instance);

		object[] parameters = new object[]{ playerControllerId, null };
		object result = dynMethod.Invoke(value,parameters);
